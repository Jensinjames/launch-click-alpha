        ...
        <div className="flex flex-col h-full">
          ...
          <div className="flex items-center px-6 py-8 border-b border-border/50">
            <div className="p-2.5 bg-gradient-to-r from-primary to-primary/80 rounded-xl shadow-lg">
              <Zap className="h-7 w-7 text-primary-foreground object-contain" />
            </div>
          ...
          <nav role="navigation" aria-label="Main menu" className="flex-1 space-y-2 py-[21px] my-0 px-0">
            {navigation.map(item => <SidebarItem key={item.name} name={item.name} href={item.href} icon={item.icon} featureName={item.featureName} isActive={isActive(item.href)} onNavigate={() => setIsOpen(false)} />)}
          </nav>
